 ...
 * Real Serial, Dynamic Macros, Note Export & Baud Config.
 */
...
function renderMacros() {
  ...
  const macros = loadMacros(); list.innerHTML = '';
  macros.forEach((m, i) => {
    const btn = document.createElement('button'); btn.className = 'task-btn'; btn.style.display = 'flex'; btn.style.justifyContent = 'space-between'; btn.style.alignItems = 'center';
    btn.innerHTML = `<span>${m.name}</span><span style="font-size:9px; opacity:0.5; margin-left:8px;">✕</span>`;
    btn.onclick = (e) => { if (e.target.tagName === 'SPAN') deleteMacro(i); else { sendSerial(m.cmd); notify(`▶ ${m.name}`); NerveAudio.play('click'); } };
    list.appendChild(btn);
  });
}
function addMacro() {
  const name = document.getElementById('macro-name').value.trim(); const cmd  = document.getElementById('macro-cmd').value.trim();
  if (!name || !cmd) return notify('⚠ Preencha nome e comando');
  const macros = loadMacros(); macros.push({ name, cmd }); saveMacros(macros);
  document.getElementById('macro-name').value = ''; document.getElementById('macro-cmd').value = ''; renderMacros();
  notify(`✓ Macro "${name}" salvo`); NerveAudio.play('notif');
}
function deleteMacro(i) { const macros = loadMacros(); macros.splice(i, 1); saveMacros(macros); renderMacros(); notify('Macro removido'); }

// ── Notes ──
function newNote() {
  const name = prompt('Nome do arquivo (sem .md):', 'session-' + new Date().toISOString().slice(0,10)); if (!name) return;
  const path = `/notes/${name}.md`; STATE.fs[path] = { type: 'file', content: `# ${name}\n\n` }; STATE.fs['/notes'].content.push(`${name}.md`);
  saveFS(); STATE.activeNotePath = path; document.getElementById('notes-area').value = STATE.fs[path].content;
  document.getElementById('notes-filename').textContent = `${name}.md`; notify(`✓ ${name}.md criado`);
}
function exportNote() {
  const content = document.getElementById('notes-area')?.value || ''; if (!content.trim()) return notify('⚠ Nota vazia');
  const filename = (STATE.activeNotePath?.split('/').pop()) || 'nerveos-notes.md';
  const blob = new Blob([content], { type: 'text/markdown' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = filename; a.click(); URL.revokeObjectURL(a.href);
  notify(`↓ Exportado: ${filename}`); NerveAudio.play('notif');
}
...
async function runBoot() {
  try {
    const lineEl = document.getElementById('boot-line'); document.addEventListener('mousedown', () => NerveAudio.init(), { once: true });
    ...
    setTimeout(() => { document.getElementById('boot')?.classList.add('hidden'); document.getElementById('desktop')?.classList.remove('hidden'); initSystem(); NerveAudio.play('boot'); notify("NerveOS v0.7.0 — OPERATIONAL"); }, 400);
  } catch (e) { document.getElementById('boot')?.classList.add('hidden'); document.getElementById('desktop')?.classList.remove('hidden'); }
}
...
async function sendSerial(data) { if (!STATE.serialWriter) return notify("Error: No hardware linked", 2000); const encoder = new TextEncoder(); await STATE.serialWriter.write(encoder.encode(data + "\n")); }
...
function initSettings() {
  const wallSelect = document.getElementById('wallpaper-select'); const desktop = document.getElementById('desktop'); const audioToggle = document.getElementById('audio-toggle'); const scanlineToggle = document.getElementById('scanline-toggle'); const baudSelect = document.getElementById('baud-select');
  const wallpapers = { cyber1: 'url("https://images.unsplash.com/photo-1614850523296-d8c1af93d400?q=80&w=2070&auto=format&fit=crop")', cyber2: 'url("https://images.unsplash.com/photo-1451187580459-43490279c0fa?q=80&w=2072&auto=format&fit=crop")', cyber3: 'url("https://images.unsplash.com/photo-1550745165-9bc0b252726f?q=80&w=2070&auto=format&fit=crop")', cinema1: 'url("https://images.unsplash.com/photo-1518770660439-4636190af475?q=80&w=2070&auto=format&fit=crop")', cinema2: 'url("https://images.unsplash.com/photo-1555680202-c86f0e12f086?q=80&w=2070&auto=format&fit=crop")' };
  ...
  const savedBaud = localStorage.getItem('nerve_baud') || '115200'; if (baudSelect) baudSelect.value = savedBaud;
...
function initSystem() {
  ...
  const unlockBtn = document.getElementById('btn-unlock'); if (unlockBtn) unlockBtn.addEventListener('click', () => { document.getElementById('lock-screen').classList.add('hidden'); NerveAudio.play('click'); });
  openWindow('terminal');
}
